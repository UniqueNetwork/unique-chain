...
describe('Matcher contract usage', () => {
  itWeb3('With UNQ', async ({api, web3}) => {
    ...
    const matcherJSON = JSON.parse((await readFile(`${__dirname}/MarketPlace.json`)).toString());
    const matcherContract = new web3.eth.Contract(matcherJSON.abi, undefined, {
      from: matcherOwner,
      ...GAS_ARGS,
    });
    ...
    // Ask
    {
      ...
      await executeEthTxOnSub(api, seller, matcher, m => m.addAsk(PRICE, '0x0000000000000000000000000000000000000001', evmCollection.options.address, tokenId, 1));
    }
...
/*   itWeb3('With custom ERC20', async ({api, web3}) => {
    ...
    {
      await executeEthTxOnSub(api, seller, evmCollection, m => m.approve(matcher.options.address, tokenId));
      await executeEthTxOnSub(api, seller, matcher, m => m.addAsk(PRICE, evmFungible.options.address, evmCollection.options.address, tokenId, 1));
    }
  ...
  itWeb3('With escrow', async ({api, web3}) => {
    ...
    {
      await executeEthTxOnSub(api, seller, evmCollection, m => m.approve(matcher.options.address, tokenId));
      await executeEthTxOnSub(api, seller, matcher, m => m.addAsk(PRICE, ksmToken, evmCollection.options.address, tokenId, 1));
    }
