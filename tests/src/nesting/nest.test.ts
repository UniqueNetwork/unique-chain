...
import {UniqueNFToken, UniqueRFToken} from '../util/playgrounds/unique';
...
describe('Integration Test: Various token type nesting', () => {
  ...
  before(async () => {
    await usingPlaygrounds(async (helper, privateKey) => {
      const donor = await privateKey({filename: __filename});
      [alice, bob, charlie] = await helper.arrange.createAccounts([100n, 20n, 20n], donor);
    });
  });
  ...
  async function checkNestedRft({
    ...
    nestedRft: UniqueRFToken,
    targetNft: UniqueNFToken,
  }) {
    const balance = await nestedRft.getBalance(targetNft.nestingAccount());
    ...
    if (childrenShouldPresent) {
      expect(children[0]).to.be.deep.equal({
        collectionId: nestedRft.collectionId,
        tokenId: nestedRft.tokenId,
      });
    } else {
  ...
  itSub.ifWithPallets('ReFungible: allows a collection owner to transfer nested token', [Pallets.ReFungible], async ({helper}) => {
    const collectionNFT = await helper.nft.mintCollection(alice);
    const collectionRFT = await helper.rft.mintCollection(alice, {
      limits: {
    ...
    await collectionNFT.setPermissions(alice, {nesting: {tokenOwner: true}});

    const targetNft = await collectionNFT.mintToken(alice, {Substrate: charlie.address});
    const nestedRft = await collectionRFT.mintToken(alice, 5n, {Substrate: charlie.address});

    await nestedRft.transfer(charlie, targetNft.nestingAccount(), 5n);
    await checkNestedRft({
      expectedBalance: 5n,
    ...
    await nestedRft.transferFrom(alice, targetNft.nestingAccount(), {Substrate: bob.address}, 2n);
    await checkNestedRft({
      expectedBalance: 3n,
      childrenShouldPresent: true,
      nestedRft,
    ...
    expect(await nestedRft.getBalance({Substrate: bob.address})).to.be.equal(2n);

    await nestedRft.transferFrom(alice, targetNft.nestingAccount(), {Substrate: bob.address}, 3n);
    await checkNestedRft({
      expectedBalance: 0n,
      childrenShouldPresent: false,
      nestedRft,
      targetNft,
    });
    expect(await nestedRft.getBalance({Substrate: bob.address})).to.be.equal(5n);
  });

  itSub.ifWithPallets('ReFungible: allows a collection admin to transfer nested token', [Pallets.ReFungible], async ({helper}) => {
    const collectionNFT = await helper.nft.mintCollection(alice);
    const collectionRFT = await helper.rft.mintCollection(alice, {
      limits: {
        ownerCanTransfer: true,
    ...
    await collectionRFT.addAdmin(alice, {Substrate: bob.address});
    ...
    await nestedRft.transferFrom(bob, targetNft.nestingAccount(), {Substrate: bob.address}, 2n);
    await checkNestedRft({
      expectedBalance: 3n,
      childrenShouldPresent: true,
      nestedRft,
    ...
    await nestedRft.transferFrom(bob, targetNft.nestingAccount(), {Substrate: bob.address}, 3n);
    await checkNestedRft({
      expectedBalance: 0n,
      childrenShouldPresent: false,
      nestedRft,
  ...
  itSub.ifWithPallets('ReFungible: allows a collection owner to burn nested token', [Pallets.ReFungible], async ({helper}) => {
    const collectionNFT = await helper.nft.mintCollection(alice, {
      limits: {
        ownerCanTransfer: true,
      },
    ...
    await nestedRft.burnFrom(alice, targetNft.nestingAccount(), 2n);
    await checkNestedRft({
      expectedBalance: 3n,
      childrenShouldPresent: true,
      nestedRft,
    ...
    await nestedRft.burnFrom(alice, targetNft.nestingAccount(), 3n);
    await checkNestedRft({
      expectedBalance: 0n,
      childrenShouldPresent: false,
      nestedRft,
    ...
    // Check if we can burn target NFT when all nested RFTs are gone
    await targetNft.burnFrom(alice, {Substrate: charlie.address});
  });

  itSub.ifWithPallets('ReFungible: allows a collection admin to burn nested token', [Pallets.ReFungible], async ({helper}) => {
    const collectionNFT = await helper.nft.mintCollection(alice, {
      limits: {
        ownerCanTransfer: true,
      },
    ...
    await collectionNFT.addAdmin(alice, {Substrate: bob.address});
    await collectionRFT.addAdmin(alice, {Substrate: bob.address});
    ...
    await nestedRft.burnFrom(bob, targetNft.nestingAccount(), 2n);
    await checkNestedRft({
      expectedBalance: 3n,
      childrenShouldPresent: true,
      nestedRft,
    ...
    await nestedRft.burnFrom(bob, targetNft.nestingAccount(), 3n);
    await checkNestedRft({
      expectedBalance: 0n,
      childrenShouldPresent: false,
      nestedRft,
    ...
    // Check if we can burn target NFT when all nested RFTs are gone
    await targetNft.burnFrom(bob, {Substrate: charlie.address});
  });
});
