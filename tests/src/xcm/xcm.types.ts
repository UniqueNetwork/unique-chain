...
export const expectFailedToTransact = async (helper: DevUniqueHelper, messageHash: string | undefined) => {
  await helper.wait.expectEvent(maxWaitBlocks, helper.getApi().events.xcmpQueue.Fail, event => event.messageHash.unwrapOr(null)?.toUtf8() == messageHash
...
export const expectDownwardXcmNoPermission = async (helper: DevUniqueHelper) => {
  ...
  await helper.wait.expectEvent(maxWaitBlocks, helper.getApi().events.dmpQueue.ExecutedDownward, event => event.outcome.asIncomplete[1].isNoPermission);
};

export const expectDownwardXcmComplete = async (helper: DevUniqueHelper) => {
  // The correct messageHash for downward messages can't be reliably obtained
  await helper.wait.expectEvent(maxWaitBlocks, helper.getApi().events.dmpQueue.ExecutedDownward, event => event.outcome.isComplete);
};
...
export class XcmTestHelper {
    ...
    await usingPlaygrounds(async (helper) => {
      ...
      */
        if(networkName == 'polkadex') {
          ...
          await helper.wait.expectEvent(maxWaitBlocks, helper.getApi().events.xcmpQueue.Success, event => event.messageHash == messageSent.messageHash);
        }
      });
    ...
    await usingPlaygrounds(async (helper) => {
      ...
      await targetPlayground(networkUrl, async (helper) => {
        if('getSudo' in helper) {
          ...
          xcmProgramSent = await helper.wait.expectEvent(maxWaitBlocks, helper.getApi().events.xcmpQueue.XcmpMessageSent);
      ...
      await helper.wait.expectEvent(maxWaitBlocks, helper.getApi().events.xcmpQueue.Success, event => event.messageHash == xcmProgramSent.messageHash);
    ...
    await usingPlaygrounds(async (helper) => {
      ...
      await targetPlayground(networkUrl, async (helper) => {
        if('getSudo' in helper) {
          ...
          maliciousXcmProgramSent = await helper.wait.expectEvent(maxWaitBlocks, helper.getApi().events.xcmpQueue.XcmpMessageSent);
        } else if('fastDemocracy' in helper) {
    ...
    await usingPlaygrounds(async (helper) => {
      ...
      await targetPlayground(networkUrl, async (helper) => {
        if('getSudo' in helper) {
          ...
          messageSent = await helper.wait.expectEvent(maxWaitBlocks, helper.getApi().events.xcmpQueue.XcmpMessageSent);
        } else if('fastDemocracy' in helper) {
          //const xcmSend = helper.constructApiCall('api.tx.polkadotXcm.send', [this._runtimeVersionedMultilocation(), maliciousXcmProgramFullId]);
          // Needed to bypass the call filter.
          //const batchCall = helper.encodeApiCall('api.tx.utility.batch', [[xcmSend]]);
  ...
  async relayIsPermittedToSetStorage(relaySudoer: IKeyringPair, variant: 'plain' | 'batch' | 'batchAll' | 'forceBatch') {
    ...
    await usingPlaygrounds(async (helper) => {
      ...
      for(const kv of kvs) {
        const forcedValue = await helper.callRpc('api.rpc.state.getStorage', [kv.key]) as Context;
