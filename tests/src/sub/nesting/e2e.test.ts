...
describe('Composite nesting tests', () => {
  ...
  itSub('Checks token children e2e', async ({helper}) => {
    ...
    expect(await targetToken.getChildren()).to.have.deep.members([
      {token: tokenA.tokenId, collection: collectionA.collectionId},
    ]).and.has.length(1);
    ...
    expect(await targetToken.getChildren()).to.have.deep.members([
      {token: tokenA.tokenId, collection: collectionA.collectionId},
      {token: tokenB.tokenId, collection: collectionA.collectionId},
    ]).and.has.length(2);
    ...
    expect(await targetToken.getChildren()).to.have.deep.members([
      {token: tokenA.tokenId, collection: collectionA.collectionId},
      {token: 0, collection: collectionB.collectionId},
    ]).and.has.length(2);
    ...
    expect(await targetToken.getChildren()).to.have.deep.members([
      {token: tokenA.tokenId, collection: collectionA.collectionId},
      {token: 0, collection: collectionB.collectionId},
      {token: tokenC.tokenId, collection: collectionC.collectionId},
    ]).and.has.length(3);
