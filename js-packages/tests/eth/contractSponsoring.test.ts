...
describe('Sponsoring Fee Limit', () => {
  ...
  itEth('Negative test - check that eth transactions exceeding fee limit are not executed', async ({helper}) => {
    ...
    await (await (<Contract>testContract.connect(user)).test.send(100, {gasLimit: 2_000_000, maxFeePerGas: gasPrice})).wait(...waitParams);
    expect(await helper.balance.getEthereum(user)).to.be.equal(originalUserBalance);

    await (await (<Contract>testContract.connect(user)).test.send(100, {gasLimit: 2_100_000, maxFeePerGas: gasPrice})).wait(...waitParams);
