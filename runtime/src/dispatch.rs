...
use pallet_common::{
	CollectionById, CollectionHandle, CommonCollectionOperations,
...
use crate::{Runtime, AccountId, CrossAccountId};

pub enum CollectionDispatchT {
	Fungible(FungibleHandle<Runtime>),
	Nonfungible(NonfungibleHandle<Runtime>),
	Refungible(RefungibleHandle<Runtime>),
}
impl CollectionDispatch<Runtime> for CollectionDispatchT {
	fn create(sender: AccountId, data: CreateCollectionData<AccountId>) -> DispatchResult {
		let _id = match data.mode {
			CollectionMode::NFT => <PalletNonfungible<Runtime>>::init_collection(sender, data)?,
			CollectionMode::Fungible(decimal_points) => {
				// check params
				ensure!(
					decimal_points <= MAX_DECIMAL_POINTS,
					pallet_unique::Error::<Runtime>::CollectionDecimalPointLimitExceeded
				);
				<PalletFungible<Runtime>>::init_collection(sender, data)?
			}
			CollectionMode::ReFungible => {
				<PalletRefungible<Runtime>>::init_collection(sender, data)?
			}
		};
		Ok(())
	}

	fn destroy(sender: CrossAccountId, collection: CollectionHandle<Runtime>) -> DispatchResult {
	...
	fn dispatch(handle: CollectionHandle<Runtime>) -> Self {
		match handle.mode {
	...
	fn into_inner(self) -> CollectionHandle<Runtime> {
		match self {
	...
	fn as_dyn(&self) -> &dyn CommonCollectionOperations<Runtime> {
		match self {
			Self::Fungible(h) => h,
...
impl pallet_evm::OnMethodCall<Runtime> for CollectionDispatchT {
	fn is_reserved(target: &H160) -> bool {
		map_eth_to_id(target).is_some()
	}
	fn is_used(target: &H160) -> bool {
		map_eth_to_id(target)
			.map(<CollectionById<Runtime>>::contains_key)
			.unwrap_or(false)
	}
	fn get_code(target: &H160) -> Option<Vec<u8>> {
		if let Some(collection_id) = map_eth_to_id(target) {
			let collection = <CollectionById<Runtime>>::get(collection_id)?;
			Some(
				match collection.mode {
					CollectionMode::NFT => <NonfungibleHandle<Runtime>>::CODE,
					CollectionMode::Fungible(_) => <FungibleHandle<Runtime>>::CODE,
					CollectionMode::ReFungible => <RefungibleHandle<Runtime>>::CODE,
				}
				.to_owned(),
			)
		} else if let Some((collection_id, _token_id)) =
			<Runtime as pallet_common::Config>::EvmTokenAddressMapping::address_to_token(target)
		{
			let collection = <CollectionById<Runtime>>::get(collection_id)?;
			...
			Some(<RefungibleTokenHandle<Runtime>>::CODE.to_owned())
		} else {
			None
		}
	}
	fn call(
		...
		if let Some(collection_id) = map_eth_to_id(target) {
			let collection =
				<CollectionHandle<Runtime>>::new_with_gas_limit(collection_id, gas_limit)?;
